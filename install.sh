...
if [ ! -f "$CONFIG_DIR/logserver.conf" ]; then
    {
        echo "# downlink logserver config - generated by installer"
        echo "addr = :8080"
...
# --- 6. systemd unit ---
cat > /etc/systemd/system/downlink.service << EOF
[Unit]
Description=downlink log server
...
systemctl enable --quiet downlink
systemctl restart downlink
sleep 1

echo ""
if systemctl is-active --quiet downlink; then
    bold "Dashboard available at :8080"
    echo "  user: admin"
    echo "  pass: $ADMIN_PASS"
    echo "  (saved in $CONFIG_DIR/logserver.conf)"
else
    echo "Service failed to start - check: journalctl -u downlink -n 50"
    exit 1
fi
